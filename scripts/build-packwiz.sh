...
packwiz curseforge export --side server --output "../../${BUILD_DIR}/MouCraft-curseforge-server.zip"
popd > /dev/null

echo "🚀 Exporting CurseForge client pack..."
pushd "${CURSEFORGE_DIR}" > /dev/null
packwiz curseforge export --side client --output "../../${BUILD_DIR}/MouCraft-curseforge-client.zip"
popd > /dev/null

echo "🚀 Exporting Modrinth pack..."
pushd "${MODRINTH_DIR}" > /dev/null
packwiz modrinth export --output "../../${BUILD_DIR}/MouCraft-modrinth.mrpack"
popd > /dev/null
